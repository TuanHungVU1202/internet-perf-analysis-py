...
class FT1_PS3:
    def plot(self):
        ...
        plt.scatter(orig_data['conn_#'], orig_data['total_bytes'])
        plt.title("Traffic volume")
        plt.xlabel("Connection Number")
        plt.ylabel("Traffic volume (bytes)")

        traffic_vol_log = plt.figure("1.12. Traffic volume - Log Scale")
        plt.scatter(orig_data['conn_#'], np.log(orig_data['total_bytes']))
